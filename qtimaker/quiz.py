...
class TextRegion(object):
    ...
    def set_title(self, text: str):
        if self.title_raw is not None:
            raise QtiMakerError('Text title has already been set')
        if self.text_raw is not None:
            raise QtiMakerError('Must set text title before text itself')
        self.title_raw = text
    ...
    def set_text(self, text: str):
        if self.text_raw is not None:
            raise QtiMakerError('Text has already been set')
        self.text_raw = text
...
class Choice(object):
    ...
    def append_feedback(self, text: str):
        if self.shortans:
            raise QtiMakerError('Feedback cannot be specified for individual short answer responses, only for the question')
        if self.feedback_raw is not None:
            raise QtiMakerError('Feedback can only be specified once')
...
class Question(object):
    ...
    def __init__(self, text: str, *, quiz: 'Quiz', title: Optional[str], points: Optional[str], md: Markdown):
        ...
        if points is None:
            ...
            try:
                ...
                raise QtiMakerError(f'Invalid points value "{points}"; need positive integer or half-integer')
            if points_num <= 0:
                raise QtiMakerError(f'Invalid points value "{points}"; need positive integer or half-integer')
    ...
    def append_feedback(self, text: str):
        if self.type is not None and not self.choices:
            raise QtiMakerError('Question feedback must immediately follow the question')
        if not self.choices:
            if self.feedback_raw is not None:
                raise QtiMakerError('Feedback can only be specified once')
            self.feedback_raw = text
    ...
    def append_correct_feedback(self, text: str):
        if self.type is not None:
            raise QtiMakerError('Correct feedback can only be specified for questions')
    ...
    def append_incorrect_feedback(self, text: str):
        if self.type is not None:
            raise QtiMakerError('Incorrect feedback can only be specified for questions')
    ...
    def append_solution(self, text: str):
        if self.type is not None:
            raise QtiMakerError('Solutions can only be specified for questions')
        if self.solution is not None:
            raise QtiMakerError('Solutions can only be specified once')
        if self.quiz.feedback_is_solution:
            raise QtiMakerError('Solutions syntax with "!" is disabled for quizzes with setting "feedback is solution: true"')
        self.solution = text
    ...
    def append_mctf_correct_choice(self, text: str):
        if self.type is None:
            ...
            raise QtiMakerError(f'Question type "{self.type}" does not support multiple choice')
        ...
        if choice.choice_html_xml in self._choice_set:
            raise QtiMakerError('Duplicate choice for question')
    ...
    def append_shortans_correct_choice(self, text: str):
        if self.type is None:
            ...
            raise QtiMakerError(f'Question type "{self.type}" does not support short answer')
    ...
    def append_multans_correct_choice(self, text: str):
        if self.type is None:
            ...
            raise QtiMakerError(f'Question type "{self.type}" does not support multiple answers')
        choice = Choice(text, correct=True, question_hash_digest=self.hash_digest, md=self.md)
        if choice.choice_html_xml in self._choice_set:
            raise QtiMakerError('Duplicate choice for question')
        self._choice_set.add(choice.choice_html_xml)
    ...
    def append_essay(self, text: str):
        ...
        if self.type is not None:
            if self.type == 'essay_question':
                raise QtiMakerError(f'Cannot specify essay response multiple times')
            raise QtiMakerError(f'Question type "{self.type}" does not support essay response')
        self.type = 'essay_question'
        if any(x is not None for x in (self.correct_feedback_raw, self.incorrect_feedback_raw)):
            raise QtiMakerError(f'Question type "{self.type}" does not support correct/incorrect feedback')
    ...
    def append_upload(self, text: str):
        ...
        if self.type is not None:
            if self.type == 'file_upload_question':
                raise QtiMakerError(f'Cannot specify upload response multiple times')
            raise QtiMakerError(f'Question type "{self.type}" does not support upload response')
    ...
    def append_numerical(self, text: str):
        if self.type is not None:
            if self.type == 'numerical_question':
                raise QtiMakerError(f'Cannot specify numerical response multiple times')
            raise QtiMakerError(f'Question type "{self.type}" does not support numerical response')
        self.type = 'numerical_question'
        self.numerical_raw = text
        if text.startswith('['):
            if not text.endswith(']') or ',' not in text:
                raise QtiMakerError('Invalid numerical response; need "[<min>, <max>]" or "<number> +- <margin>" or "<integer>"')
            ...
            try:
                ...
                raise QtiMakerError('Invalid numerical response; need "[<min>, <max>]" or "<number> +- <margin>" or "<integer>"')
            if min > max:
                raise QtiMakerError('Invalid numerical response; need "[<min>, <max>]" with min < max')
            ...
            try:
                ...
                raise QtiMakerError('Invalid numerical response; need "[<min>, <max>]" or "<number> +- <margin>" or "<integer>"')
            if margin < 0:
                raise QtiMakerError('Invalid numerical response; need "<number> +- <margin>" with margin > 0')
        ...
        if abs(min) < 1e-4 or abs(max) < 1e-4:
            raise QtiMakerError('Invalid numerical response; all acceptable values must have a magnitude >= 0.0001')


    def finalize(self):
        if self.type is None:
            raise QtiMakerError('Question must specify a response type')
        elif self.type == 'multiple_choice_question':
            if len(self.choices) == 2 and all(c.choice_raw in ('true', 'True', 'false', 'False') for c in self.choices):
                self.type = 'true_false_question'
            if not self.choices:
                raise QtiMakerError('Question must provide choices')
            if len(self.choices) < 2:
                raise QtiMakerError('Question must provide more than one choice')
            if self.correct_choices < 1:
                raise QtiMakerError('Question must specify a correct choice')
            if self.correct_choices > 1:
                raise QtiMakerError('Question must specify only one correct choice')
        elif self.type == 'short_answer_question':
            if not self.choices:
                raise QtiMakerError('Question must provide at least one answer')
        elif self.type == 'multiple_answers_question':
            if len(self.choices) < 2:
                raise QtiMakerError('Question must provide more than one choice')
            if self.correct_choices < 1:
...
class Group(object):
    ...
    def append_group_pick(self, text: str):
        if self.questions:
            raise QtiMakerError('Question group options must be set at the very start of the group')
        if self._pick_is_set:
            QtiMakerError('"Pick" has already been set for this question group')
        try:
            self.pick = int(text)
        except Exception as e:
            raise QtiMakerError(f'"pick" value is invalid (must be positive number):\n{e}')
        if self.pick <= 0:
            raise QtiMakerError('"pick" value is invalid (must be positive number)')
        if self.solutions_pick is not None and self.pick > self.solutions_pick:
            raise QtiMakerError('"pick" value must be less than or equal to "solutions pick" value')
    ...
    def append_group_solutions_pick(self, text: str):
        ...
        if self.solutions_pick is not None:
            QtiMakerError('"solutions pick" has already been set for this question group')
        try:
            self.solutions_pick = int(text)
        except Exception as e:
            raise QtiMakerError(f'"solutions pick" value is invalid (must be positive number):\n{e}')
        if self.solutions_pick <= 0:
            raise QtiMakerError('"solutions pick" value is invalid (must be positive number)')
        if self.solutions_pick < self.pick:
            raise QtiMakerError('"solutions pick" value must be greater than or equal to "pick" value')

    def append_group_points_per_question(self, text: str):
        if self.questions:
            raise QtiMakerError('Question group options must be set at the very start of the group')
        if self._points_per_question_is_set:
            QtiMakerError('"Points per question" has already been set for this question group')
        try:
            self.points_per_question = int(text)
        except Exception as e:
            raise QtiMakerError(f'"Points per question" value is invalid (must be positive number):\n{e}')
    ...
    def append_question(self, question: Question):
        if self._question_points_possible is None:
            ...
            raise QtiMakerError('Question groups must only contain questions with the same point value')
        self.questions.append(question)

    def finalize(self):
        if len(self.questions) < self.pick:
            raise QtiMakerError(f'Question group only contains {len(self.questions)} questions, needs at least {self.pick+1}')
        if self.solutions_pick is not None and len(self.questions) < self.solutions_pick:
            raise QtiMakerError(f'Question group only contains {len(self.questions)} questions, needs at least {self.solutions_pick}')
        h = hashlib.blake2b()
...
class Quiz(object):
    ...
    def __init__(self, string: str, *, config: Config,
        ...
        if resource_path is not None:
            ...
            if not resource_path.is_dir():
                raise QtiMakerError(f'Resource path "{resource_path.as_posix()}" does not exist')
        ...
        try:
                    ...
                    if action == 'start_code':
                        ...
                        if info_match is None:
                            ...
                            while line is not None and not (line.startswith(delim) and line[len(delim):] == line.lstrip('`')):
                                ...
                                n, line = next(n_line_iter, (0, None))
                            if line is None:
                                raise QtiMakerError(f'In {self.source_name} on line {n}:\nCode closing fence is missing')
                            if line.lstrip('`').strip():
                                raise QtiMakerError(f'In {self.source_name} on line {n+1}:\nCode closing fence is missing')
                            code_lines.append('\n')
                            code = '\n'.join(code_lines)
                            try:
                                stdout = self._run_code(executable, code)
                            except Exception as e:
                                raise QtiMakerError(f'In {self.source_name} on line {n_code_start+1}:\n{e}')
            ...
            if not self.questions_and_delims:
                raise QtiMakerError('No questions were found')
            if self._current_group is not None:
                raise QtiMakerError(f'In {self.source_name} on line {len(string.splitlines())}:\nQuestion group never ended')
    ...
    def _run_code(self, executable: str, code: str) -> str:
        if not self.config['run_code_blocks']:
            raise QtiMakerError('Code execution for code blocks is not enabled; use --run-code-blocks, or set run_code_blocks = true in config')
        h = hashlib.blake2b()
        ...
        with tempfile.TemporaryDirectory() as tempdir:
            ...
            if platform.system() == 'Windows':
                ...
                if which_executable is None:
                    raise QtiMakerError(f'Failed to execute code (missing executable "{executable}")')
            ...
            try:
                ...
                raise QtiMakerError(f'Failed to execute code with command "{cmd}":\n{e}')
        ...
        if proc.returncode != 0:
            ...
            raise QtiMakerError(f'Code execution resulted in errors:\n{"-"*50}\n{stderr_str}\n{"-"*50}')
        try:
            stdout_str = io.TextIOWrapper(io.BytesIO(proc.stdout),
                                          encoding=locale.getpreferredencoding(False)).read()
        except Exception as e:
            raise QtiMakerError(f'Failed to decode output of executed code:\n{e}')
        return stdout_str
    ...
    def append_quiz_title(self, text: str):
        if any(x is not None for x in (self.shuffle_answers_raw, self.show_correct_answers_raw,
            ...
            raise QtiMakerError('Must give quiz title before quiz options')
        if self._next_question_attr:
            raise QtiMakerError('Expected question; question title and/or points were set but not used')
        if self.title_raw is not None:
            raise QtiMakerError('Quiz title has already been given')
        if self.questions_and_delims:
            raise QtiMakerError('Must give quiz title before questions')
        if self.description_raw is not None:
            raise QtiMakerError('Must give quiz title before quiz description')
        self.title_raw = text
        self.title_xml = self.md.xml_escape(text)
    ...
    def append_quiz_description(self, text: str):
        if any(x is not None for x in (self.shuffle_answers_raw, self.show_correct_answers_raw,
                                       self.one_question_at_a_time_raw, self.cant_go_back_raw)):
            raise QtiMakerError('Must give quiz description before quiz options')
        if self._next_question_attr:
            raise QtiMakerError('Expected question; question title and/or points were set but not used')
        if self.description_raw is not None:
            raise QtiMakerError('Quiz description has already been given')
        if self.questions_and_delims:
            raise QtiMakerError('Must give quiz description before questions')
    ...
    def append_quiz_shuffle_answers(self, text: str):
        if self._next_question_attr:
            raise QtiMakerError('Expected question; question title and/or points were set but not used')
        if self.questions_and_delims:
            raise QtiMakerError('Must give quiz options before questions')
        if self.shuffle_answers_raw is not None:
            raise QtiMakerError('Quiz option "Shuffle answers" has already been set')
        if text not in ('true', 'True', 'false', 'False'):
            raise QtiMakerError('Expected option value "true" or "false"')
    ...
    def append_quiz_show_correct_answers(self, text: str):
        ...
        if self.show_correct_answers_raw is not None:
            raise QtiMakerError('Quiz option "Show correct answers" has already been set')
        if text not in ('true', 'True', 'false', 'False'):
            raise QtiMakerError('Expected option value "true" or "false"')
    ...
    def append_quiz_one_question_at_a_time(self, text: str):
        ...
        if self.one_question_at_a_time_raw is not None:
            raise QtiMakerError('Quiz option "One question at a time" has already been set')
        if text not in ('true', 'True', 'false', 'False'):
            raise QtiMakerError('Expected option value "true" or "false"')
    ...
    def append_quiz_cant_go_back(self, text: str):
        ...
        if self.cant_go_back_raw is not None:
            raise QtiMakerError('''Quiz option "Can't go back" has already been set''')
        if text not in ('true', 'True', 'false', 'False'):
            raise QtiMakerError('Expected option value "true" or "false"')
        if self.one_question_at_a_time_xml != 'true':
            raise QtiMakerError('''Must set "One question at a time" to "true" before setting "Can't go back"''')
    ...
    def append_quiz_feedback_is_solution(self, text: str):
        ...
        if self.feedback_is_solution is not None:
            raise QtiMakerError('Quiz option "feedback is solution" has already been set')
    ...
    def append_quiz_solutions_sample_groups(self, text: str):
        ...
        if self.solutions_sample_groups is not None:
            raise QtiMakerError('Quiz option "solutions sample groups" has already been set')
        if text in ('true', 'True'):
    ...
    def append_quiz_solutions_randomize_groups(self, text: str):
        ...
        if self.solutions_randomize_groups is not None:
            raise QtiMakerError('Quiz option "solutions randomize groups" has already been set')
        if text in ('true', 'True'):
    ...
    def append_question(self, text: str):
        ...
        if question.question_html_xml in self.question_set:
            raise QtiMakerError('Duplicate question')
    ...
    def append_question_title(self, text: str):
        if 'title' in self._next_question_attr:
            raise QtiMakerError('Title for next question has already been set')
        if 'points' in self._next_question_attr:
            raise QtiMakerError('Title for next question must be set before point value')
        self._next_question_attr['title'] = text

    def append_question_points(self, text: str):
        if 'points' in self._next_question_attr:
            raise QtiMakerError('Points for next question has already been set')
    ...
    def append_feedback(self, text: str):
        if self._next_question_attr:
            raise QtiMakerError('Expected question; question title and/or points were set but not used')
        if not self.questions_and_delims:
            raise QtiMakerError('Cannot have feedback without a question')
        last_question_or_delim = self.questions_and_delims[-1]
    ...
    def append_solution(self, text: str):
        if self.feedback_is_solution:
            raise QtiMakerError('Quiz option "feedback is solution" is "true"; '
                                '"!" solution syntax is disabled and solutions must be provided as feedback ("...") rather than separately')
        if self._next_question_attr:
            raise QtiMakerError('Expected question; question title and/or points were set but not used')
        if not self.questions_and_delims:
            raise QtiMakerError('Cannot have a solution without a question')
        last_question_or_delim = self.questions_and_delims[-1]
        if not isinstance(last_question_or_delim, Question):
            raise QtiMakerError('Cannot have a solution without a question')
    ...
    def append_mctf_correct_choice(self, text: str):
        if self._next_question_attr:
            raise QtiMakerError('Expected question; question title and/or points were set but not used')
        if not self.questions_and_delims:
            raise QtiMakerError('Cannot have a choice without a question')
        last_question_or_delim = self.questions_and_delims[-1]
        if not isinstance(last_question_or_delim, Question):
            raise QtiMakerError('Cannot have a choice without a question')
    ...
    def append_shortans_correct_choice(self, text: str):
        if self._next_question_attr:
            raise QtiMakerError('Expected question; question title and/or points were set but not used')
        if not self.questions_and_delims:
            raise QtiMakerError('Cannot have an answer without a question')
        last_question_or_delim = self.questions_and_delims[-1]
        if not isinstance(last_question_or_delim, Question):
            raise QtiMakerError('Cannot have an answer without a question')
    ...
    def append_essay(self, text: str):
        if self._next_question_attr:
            raise QtiMakerError('Expected question; question title and/or points were set but not used')
        if not self.questions_and_delims:
            raise QtiMakerError('Cannot have an essay response without a question')
        last_question_or_delim = self.questions_and_delims[-1]
        if not isinstance(last_question_or_delim, Question):
            raise QtiMakerError('Cannot have an essay response without a question')
    ...
    def append_upload(self, text: str):
        if self._next_question_attr:
            raise QtiMakerError('Expected question; question title and/or points were set but not used')
        if not self.questions_and_delims:
            raise QtiMakerError('Cannot have an upload response without a question')
        last_question_or_delim = self.questions_and_delims[-1]
        if not isinstance(last_question_or_delim, Question):
            raise QtiMakerError('Cannot have an upload response without a question')
    ...
    def append_numerical(self, text: str):
        if self._next_question_attr:
            raise QtiMakerError('Expected question; question title and/or points were set but not used')
        if not self.questions_and_delims:
            raise QtiMakerError('Cannot have a numerical response without a question')
        last_question_or_delim = self.questions_and_delims[-1]
        if not isinstance(last_question_or_delim, Question):
            raise QtiMakerError('Cannot have a numerical response without a question')
    ...
    def append_start_group(self, text: str):
        ...
        if text:
            raise ValueError
        if self._current_group is not None:
            raise QtiMakerError('Question groups cannot be nested')
        if self.questions_and_delims:
            last_question_or_delim = self.questions_and_delims[-1]
            if isinstance(last_question_or_delim, Question):
                last_question_or_delim.finalize()
    ...
    def append_end_group(self, text: str):
        ...
        if self._current_group is None:
            raise QtiMakerError('No question group to end')
        if self.questions_and_delims:
            last_question_or_delim = self.questions_and_delims[-1]
            if isinstance(last_question_or_delim, Question):
                last_question_or_delim.finalize()
    ...
    def append_group_pick(self, text: str):
        if self._next_question_attr:
            raise QtiMakerError('Expected question; question title and/or points were set but not used')
        if self._current_group is None:
            raise QtiMakerError('No question group for setting properties')
    ...
    def append_start_code(self, text: str):
        raise QtiMakerError('Invalid code block start')

    def append_end_code(self, text: str):
        raise QtiMakerError('Code block end missing code block start')
    ...
    def append_unknown(self, text: str):
        ...
        if text and not text.isspace():
            ...
            if match:
                raise QtiMakerError(f'Missing whitespace after "{match.group().strip()}"')
            match = start_missing_content_re.match(text)
            if match:
                raise QtiMakerError(f'Missing content after "{match.group().strip()}"')
            raise QtiMakerError(f'Syntax error; unexpected text, or incorrect indentation for a wrapped paragraph:\n"{text}"')
