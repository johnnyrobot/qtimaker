...
def main():
    '''
    Command-line executable main function.
    '''
    parser = argparse.ArgumentParser(prog='qtimaker')
    parser.set_defaults(func=lambda x: parser.print_help())
    parser.add_argument('--version', action='version', version=f'QTI Maker {version}')
    parser.add_argument('--qti-version', choices=['1.2', '2.1'], default='1.2',
                        help='QTI version to generate: 1.2 for Classic Quizzes, 2.1 for New Quizzes (default: 1.2)')
    ...
    parser.add_argument('--pandoc-mathml', action='store_const', const=True,
                        help='Convert LaTeX math to MathML using Pandoc (this will create a cache file "_qtimaker_cache.zip" in the quiz file directory)')
    ...
    if not config.loaded_config_file and sys.stdout.isatty() and sys.stdin.isatty():
        latex_render_url = input(textwrap.dedent('''\
            It looks like this application has not been installed on this machine
    ...
    try:
        ...
        raise QtiMakerError(f'File "{file_path}" does not exist')
    except PermissionError as e:
        raise QtiMakerError(f'File "{file_path}" cannot be read due to permission error:\n{e}')
    except UnicodeDecodeError as e:
        raise QtiMakerError(f'File "{file_path}" is not encoded in valid UTF-8:\n{e}')
    ...
    if solutions_paths is not None:
        if file_path_abs in solutions_paths:
            raise QtiMakerError(f'Solutions cannot overwrite quiz file "{file_path}"')
        if not all(x.suffix.lower() in ('.md', '.markdown', '.pdf', '.html') for x in solutions_paths):
            ...
            raise QtiMakerError(f'Unsupported export format(s) {invalid_extensions} for solutions; use .md, .markdown, .pdf, or .html')
    ...
    try:
            ...
            for solutions_path in solutions_paths:
                if solutions_path.suffix.lower() == '.pdf':
                    if not shutil.which('pandoc'):
                        raise QtiMakerError('Exporting solutions in PDF format requires Pandoc (https://pandoc.org/)')
                    if not shutil.which('pdflatex'):
                        raise QtiMakerError('Exporting solutions in PDF format requires LaTeX (https://www.tug.org/texlive/ or https://miktex.org/)')
                    ...
                    try:
                        ...
                        raise QtiMakerError(f'Pandoc failed:\n{"-"*78}\n{e}\n{"-"*78}')
                elif solutions_path.suffix.lower() == '.html':
                    if not shutil.which('pandoc'):
                        raise QtiMakerError('Exporting solutions in HTML format requires Pandoc (https://pandoc.org/)')
                    if platform.system() == 'Windows':
        ...
        if qti_path is not None:
            qti = QTI(quiz, qti_version=args.qti_version)
            qti.save(qti_path)
    finally:
        os.chdir(cwd)
