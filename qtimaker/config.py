...
class Config(dict):
    ...
    _config_path = pathlib.Path('~/.qtimaker.bespon').expanduser()

    def __setitem__(self, key, value):
        if key not in self._key_check:
            raise QtiMakerError(f'Invalid configuration option "{key}"')
        if not self._key_check[key](value):
            raise QtiMakerError(f'Configuration option "{key}" has invalid value "{value}"')
    ...
    def __missing__(self, key):
        if self.loaded_config_file:
            raise QtiMakerError(textwrap.dedent(f'''\
                Configuration option "{key}" has not been set.
                Open "{self._config_path}" to edit config manually.
                '''))
        raise QtiMakerError(f'Configuration option "{key}" has not been set.')
    ...
    def load(self):
        ...
        try:
            ...
            try:
                ...
                warnings.warn(f'Could not create default config file "{config_path}" due to FileNotFoundError (directory does not exist).')
            except PermissionError:
                warnings.warn(f'Could not create default config file "{config_path}" due to PermissionError.')
        except PermissionError:
            raise QtiMakerError(f'Could not open config file "{config_path}" due to PermissionError.')
        except UnicodeDecodeError:
            raise QtiMakerError(f'Could not open config file "{config_path}" due to UnicodeDecodeError. File may be corrupt.')

        if config_text is not None:
            try:
                config_dict = bespon.loads(config_text, empty_default=dict)
            except Exception as e:
                raise QtiMakerError(f'Failed to load config file "{config_path}":\n{e}')
            try:
    ...
    def save(self):
        ...
        try:
            ...
            raise QtiMakerError(f'Failed to convert config data to config file format (invalid data?):\n{e}')
        try:
            config_path.write_text(bespon_text, 'utf8')
        except FileNotFoundError:
            raise QtiMakerError(f'Could not create config file "{config_path}" due to FileNotFoundError (directory does not exist).')
        except PermissionError:
            raise QtiMakerError(f'Could not create config file "{config_path}" due to PermissionError.')
