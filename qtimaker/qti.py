...
from .xml_imsmanifest import imsmanifest, imsmanifest_v2
from .xml_assessment_meta import assessment_meta
from .xml_assessment import assessment
from .xml_assessment_v2 import assessment_v2


class QTI(object):
    '''
    Create QTI from a Quiz object.
    Supports both QTI 1.2 (Classic Quizzes) and QTI 2.1 (New Quizzes).
    '''
    def __init__(self, quiz: Quiz, qti_version: Literal['1.2', '2.1'] = '1.2'):
        self.quiz = quiz
        ...
        if qti_version == '2.1':
            self.imsmanifest_xml = imsmanifest_v2(manifest_identifier=self.manifest_identifier,
                                                  assessment_identifier=self.assessment_identifier,
                                                  dependency_identifier=self.dependency_identifier,
                                                  images=self.quiz.images)
            self.assessment = assessment_v2(quiz=quiz,
                                           assessment_identifier=self.assessment_identifier,
                                           title_xml=quiz.title_xml)
            # QTI 2.1 doesn't use assessment_meta.xml in the same way
            self.assessment_meta = None
        else:  # QTI 1.2
    ...
    def write(self, bytes_stream: BinaryIO):
        with zipfile.ZipFile(bytes_stream, 'w', compression=zipfile.ZIP_DEFLATED) as zf:
            ...
            if self.qti_version == '1.2':
                zf.writestr(zipfile.ZipInfo('non_cc_assessments/'), b'')
                if self.assessment_meta is not None:
