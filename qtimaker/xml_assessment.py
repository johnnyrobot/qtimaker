...
def assessment(*, quiz: Quiz, assessment_identifier: str, title_xml: str) -> str:
    ...
    for question_or_delim in quiz.questions_and_delims:
        if isinstance(question_or_delim, TextRegion):
            xml.append(TEXT.format(ident=f'qtimaker_text_{question_or_delim.id}',
                                   text_title_xml=question_or_delim.title_xml,
                                   assessment_question_identifierref=f'qtimaker_question_ref_{question_or_delim.id}',
        ...
        if question.type in ('true_false_question', 'multiple_choice_question',
            ...
            original_answer_ids = ','.join(f'qtimaker_choice_{c.id}' for c in question.choices)
        elif question.type == 'numerical_question':
            item_metadata = ITEM_METADATA_MCTF_SHORTANS_MULTANS_NUM
            original_answer_ids = f'qtimaker_numerical_{question.id}'
        elif question.type == 'essay_question':
            item_metadata = ITEM_METADATA_ESSAY
            original_answer_ids = f'qtimaker_essay_{question.id}'
        elif question.type == 'file_upload_question':
            item_metadata = ITEM_METADATA_UPLOAD
            original_answer_ids = f'qtimaker_upload_{question.id}'
        else:
            raise ValueError
        xml.append(item_metadata.format(question_type=question.type,
                                        points_possible=question.points_possible,
                                        original_answer_ids=original_answer_ids,
                                        assessment_question_identifierref=f'qtimaker_question_ref_{question.id}'))
        ...
        if question.type in ('true_false_question', 'multiple_choice_question', 'multiple_answers_question'):
            if question.type in ('true_false_question', 'multiple_choice_question'):
                ...
                raise ValueError
            choices = '\n'.join(item_presentation_choice.format(ident=f'qtimaker_choice_{c.id}', choice_html_xml=c.choice_html_xml)
        ...
        if question.type in ('true_false_question', 'multiple_choice_question'):
            ...
            for choice in question.choices:
                if choice.feedback_raw is not None:
                    resprocessing.append(ITEM_RESPROCESSING_MCTF_CHOICE_FEEDBACK.format(ident=f'qtimaker_choice_{choice.id}'))
            if question.correct_feedback_raw is not None:
                resprocessing.append(ITEM_RESPROCESSING_MCTF_SET_CORRECT_WITH_FEEDBACK.format(ident=f'qtimaker_choice_{correct_choice.id}'))
            else:
                resprocessing.append(ITEM_RESPROCESSING_MCTF_SET_CORRECT_NO_FEEDBACK.format(ident=f'qtimaker_choice_{correct_choice.id}'))
            ...
            for choice in question.choices:
                if choice.feedback_raw is not None:
                    resprocessing.append(ITEM_RESPROCESSING_SHORTANS_CHOICE_FEEDBACK.format(ident=f'qtimaker_choice_{choice.id}', answer_xml=choice.choice_xml))
            varequal = []
            for choice in question.choices:
            ...
            for choice in question.choices:
                if choice.feedback_raw is not None:
                    resprocessing.append(ITEM_RESPROCESSING_MULTANS_CHOICE_FEEDBACK.format(ident=f'qtimaker_choice_{choice.id}'))
            varequal = []
            for choice in question.choices:
                if choice.correct:
                    varequal.append(ITEM_RESPROCESSING_MULTANS_SET_CORRECT_VAREQUAL_CORRECT.format(ident=f'qtimaker_choice_{choice.id}'))
                else:
                    varequal.append(ITEM_RESPROCESSING_MULTANS_SET_CORRECT_VAREQUAL_INCORRECT.format(ident=f'qtimaker_choice_{choice.id}'))
            if question.correct_feedback_raw is not None:
        ...
        if question.type in ('true_false_question', 'multiple_choice_question',
            ...
            for choice in question.choices:
                if choice.feedback_raw is not None:
                    xml.append(ITEM_FEEDBACK_MCTF_SHORTANS_MULTANS_NUM_INDIVIDUAL.format(ident=f'qtimaker_choice_{choice.id}',
