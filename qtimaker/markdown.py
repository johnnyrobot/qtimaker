...
class QtiMakerImagePattern(ImageInlineProcessor):
    '''
    ...
    inclusion.
    '''
    def __init__(self, pattern_re, markdown_md, qtimaker_md):
        super().__init__(pattern_re, markdown_md)
        self.qtimaker_md = qtimaker_md
    ...
    def handleMatch(self, match, data):
        ...
        if src and not any(src.startswith(x) for x in ('http://', 'https://')):
            ...
            try:
                ...
                raise QtiMakerError(f'File "{src_path}" does not exist')
            except PermissionError as e:
                raise QtiMakerError(f'File "{src_path}" cannot be read due to permission error:\n{e}')
            image = Image(src_path.name, data)
            if image.id in self.qtimaker_md.images:
                image = self.qtimaker_md.images[image.id]
            else:
                if image.name in self.qtimaker_md.image_name_set:
                    n = 8
                    while image.name in self.qtimaker_md.image_name_set:
                        image.name = f'{src_path.stem}_{image.id[:n]}{src_path.suffix}'
                        n *= 2
                        if n >= len(image.id)*2:
                            raise QtiMakerError('Hash collision occurred during image deduplication')
                self.qtimaker_md.image_name_set.add(image.name)
                self.qtimaker_md.images[image.id] = image
...
class Markdown(object):
    ...
    def __init__(self, config: Optional[Config]=None):
        ...
        markdown_image_processor = QtiMakerImagePattern(IMAGE_LINK_RE, markdown_processor, self)
    ...
    def _latex_to_qti_unconfigured(self, latex: str):
        raise QtiMakerError('Cannot convert LaTeX to QTI unless Markdown configuration is provided')


    def _prep_cache(self):
        self._cache_path = pathlib.Path('_qtimaker_cache.zip')
        self._cache_lock_path = pathlib.Path('_qtimaker_cache.lock')
        ...
        while True:
            try:
                ...
                if lock_time > max_lock_wait:
                    raise QtiMakerError('The cache is locked; this usually means that another instance of '
                                        'the application is already running and you should try again later')
    ...
    def latex_to_canvas_img(self, latex: str) -> str:
        ...
        in the config file or can be passed with flag --latex-render-url.
    ...
    def latex_to_pandoc_mathml(self, latex: str) -> str:
        ...
        if data is not None:
            ...
            try:
                ...
                raise QtiMakerError(f'Could not find Pandoc:\n{e}')
            except subprocess.CalledProcessError as e:
                raise QtiMakerError(f'Running Pandoc failed:\n{e}')
    ...
    def siunitx_num_to_plain_latex(self, number: str, in_math: bool=False) -> str:
        ...
        if not self.siunitx_num_number_re.match(number):
            raise QtiMakerError(f'Invalid or unsupported LaTeX number "{number}"')
        number = number.lower()
        if 'e' in number:
        ...
        while True:
            if char == '' or char == ' ':
                ...
                if char.isdigit():
                    ...
                    raise QtiMakerError(f'Invalid or unsupported LaTeX unit "{unit}"')
            elif char == '/':
    ...
    def md_to_html_xml(self, markdown_string: str, strip_p_tags: bool=False) -> str:
        ...
        try:
            ...
            raise QtiMakerError(f'Conversion from Markdown to HTML failed:\n{e}')
        if strip_p_tags:
